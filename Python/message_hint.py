...
class ClientMessage:
    ...
    def i_am_at(self):
        ...
        '''
        return "IAMAT {0} {1} {2}".format(self.client_id, self.coordinates, time.time())
    def whats_at(self, another_client=None, radius=0, max_results=0):
        ...
        '''
        if another_client == None:
            another_client = self.client_id
        return "WHATSAT {0} {1} {2}".format(another_client, radius, max_results)
...
class ServerMessage:
    ...
    def parse_message(self, message):
        ...
        return command_table.get(message_list[0], lambda x, y, z: "ERROR: command name {} unrecognized".format(message_list[0]))(*message_list[1:])
    def handle_i_am_at(self, client_id, coordinates, timestamp):
        time.sleep(2) # sleep 2 seconds
        msg = "AT {0} +{1} {2} {3} {4}".format(self.server_name, time.time() - float(timestamp), client_id, coordinates, timestamp)
