...
class Colors(object):
    ...
    def __repr__(self):
        ...
        """
        return f"repr: color object \"{self.name}\""
    ...
    def __str__(self):
        ...
        called when converting this object into a string: (assuming color=Colors("my colors"))
            ...
            str(color)
        """
        return f"object \"{self.name}\" converted to string"
...
def create_vocabulary(word_list):
...
if __name__ == '__main__':
    ...
    for i, k in enumerate(my_dict.keys()):
        print(f"\tthe {i}-th key is {k}")
    for v, pair in zip(my_dict.values(), my_dict.items()):
        print(f"\tthe value {v} comes from the key-value pair {pair}")
    print("this is an example showing how to create a dictionary from key list and value list "
          "(and a scenario showing some other features of dict BTW)")
